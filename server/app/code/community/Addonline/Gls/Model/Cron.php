...
class Addonline_Gls_Model_Cron extends Mage_Core_Model_Config_Data
{
    ...
    public function update_gls_agencies()
    {
        ...
        $tableName = Mage::getSingleton('core/resource')->getTableName('gls_agencies_list');
        ...
        $lastImportQuery = $dbInstance->select()->from($tableName, array('last_import_date'));
        ...
        if ($forceupdate || $remoteFileDate > $lastImportDate) {
            ...
            if (count($aAgencies) > 1) {
                ...
                //On vide la table
                $dbInstance->query('TRUNCATE ' . $tableName);
                ...
                foreach ($aAgencies as $key => $agency) {
                    ...
                    $dbInstance->insert($tableName, $fields);
                    $dbInstance->commit();
                }
            }
        }
