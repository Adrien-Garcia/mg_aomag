 ...
 */
class Gls_Product implements GLS_Product_Interface {
	...
	public function Gls_Product($cart_item, $parent_cart_item) {
...
class Addonline_Gls_Model_Carrier_ShippingMethod extends Mage_Shipping_Model_Carrier_Abstract
{
  /**
  	...
  	if (!$this->__getConfigData('active')) return false;
		...
		foreach ($cart_items as $item) {
			if ($item->getProduct()->getTypeId()!='configurable') {
				...
				$magento_product = new Gls_Product($item, isset($cart_items[$parent_item_id]) ? $cart_items[$parent_item_id] : null);
  ...
  protected function _getConfig() {
  	if (!isset($this->_config)) {
  		...
  		if($this->__getConfigData('livraisontoyou')){
  			$allConfigurations .= $this->__getConfigData('configtoyou');
  		}
  		if($this->__getConfigData('livraisonrelay')){
  			$allConfigurations .= $this->__getConfigData('configrelay');
  		}

  		$this->_helper = new GlsShippingHelper($allConfigurations);
  ...
  protected function __getDefaultProcessData($store_id=null) {
  	...
  	$properties = array_merge(GlsShippingHelper::getDefaultProcessData(),array(
  			'info.magento.version' => Mage::getVersion(),
  			'info.module.version' => (string)$mage_config->getNode('modules/Addonline_Gls/version'),
