 ...
 * @category   Addonline
 * @package    Addonline_SprintSecure
 * @name       Addonline_SprintSecure_Model_Api_Standard_Response
 */
class Addonline_SprintSecure_Model_Api_Response extends Addonline_SprintSecure_Model_Api_Parameters
{
	...
	public function describeResponse($response, $return='string') 
	{
		...
		$string = Mage::helper('sprintsecure')->__('Numero de transaction : %s', $response['transaction_id']) . "\n";
		$string.= Mage::helper('sprintsecure')->__('Mode de capture : %s', $response['capture_mode']) . "\n";
		
		if (isset($response['capture_day']) && is_numeric($response['capture_day']))
		{
		    if ($response['capture_day'] == 0) {
			  $string.= Mage::helper('sprintsecure')->__('Jour avant la capture : capture immediate') . "\n";
			} else {
		      $string.= Mage::helper('sprintsecure')->__('Jour avant la capture : %s', $response['capture_day']) . "\n";
			}
		}
		
		$string.= Mage::helper('sprintsecure')->__('Type de carte de credit : %s', $response['payment_means']) . "\n";
...
// Credit card number
        if (isset($response['card_number']) && !empty($response['card_number'])) 
		{
		    ...
		    $string.= Mage::helper('sprintsecure')->__('Numero de carte bancaire : %s', $array['card_number']) . "\n";
		}
		...
		if (isset($response['cvv_flag'])) 
		{
		    switch ($response['cvv_flag']) 
			{
			    case '1':
				    ...
				    $string .= Mage::helper('sprintsecure')->__('A propos du cryptogramme de la carte : %s', $array['cvv_response_code']) . "\n";
					
					if (isset($response['cvv_key']))
					{
					    $array['cvv_key'] = $response['cvv_key'];
				        $string .= Mage::helper('sprintsecure')->__('Cryptogramme de la carte de credit : %s', $response['cvv_key']) . "\n";
					}
				    break;
			}
		}
		...
		if (isset($response['response_code']))
		{
		    ...
		    $string .= Mage::helper('sprintsecure')->__('Code reponse de la banque : %s', $array['response_code']) . "\n";
		}
	    ...
	    if (isset($response['bank_response_code'])) 
		{
		    if (in_array($response['payment_means'], array('CB', 'VISA', 'MASTERCARD')))
		    {
				...
				{
		            $string .= Mage::helper('sprintsecure')->__('Code reponse de la Banque : %s', $array['bank_response_code']) . "\n";
				}
			}
		}
		...
		if (isset($response['complementary_code']))
		{
			...
			{
		        $string .= Mage::helper('sprintsecure')->__('Controle supplementaire : %s', $array['complementary_code']) . "\n";
		    }
		}
