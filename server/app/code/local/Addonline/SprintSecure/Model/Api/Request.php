 ...
 * @name       Addonline_SprintSecure_Model_Api_Request
 */
class Addonline_SprintSecure_Model_Api_Request extends Addonline_SprintSecure_Model_Api_Parameters
{
    public function doRequest($parameters = array())
    {
		...
		if (!empty($sips_result)) {
			...
			{
		        Mage::throwException( Mage::helper('sprintsecure')->__('Impossible to launch binary file - Path to binary file seem to be not correct (%s)<br />Command line : %s', $parameters['bin_request'], $command) );
			}
			
			if (is_executable($parameters['bin_request']) === false)
			{
			    $perms = substr(sprintf('%o', fileperms($parameters['bin_request'])), -4);
			    Mage::throwException( Mage::helper('sprintsecure')->__('Impossible to execute binary file - Set correct chmod (current chmod %s)<br />Command line : %s', $perms, $command) );
			}
