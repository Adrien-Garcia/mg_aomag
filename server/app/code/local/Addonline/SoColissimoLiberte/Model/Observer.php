...
class Addonline_SoColissimoLiberte_Model_Observer extends Varien_Object
{
	...
	public function _9cd4777ae76310fd6977a5c559c51820(){
		...
		if((md5(Mage::getStoreConfig('web/unsecure/base_url').$key.'SoColissimoLiberte')!=Mage::getStoreConfig('socolissimoliberte/licence/serial')) && (Mage::getStoreConfig('socolissimoliberte/licence/active')) ){
			$severity=Mage_AdminNotification_Model_Inbox::SEVERITY_MAJOR;$title= "Vous devez renseigner une clé licence valide pour le module So colissimo Liberté. Le module a été désactivé";$description= "Le module So colissimo Liberté n'a pas une clé licence valide";	$date = date('Y-m-d H:i:s'); Mage::getModel('adminnotification/inbox')->parse(array(array('severity' => $severity,'date_added'=> $date,'title'=> $title,'description'   => $description,'url'=> '','internal'      => true)));	Mage::getModel('core/config')->saveConfig('socolissimoliberte/licence/active',0);return false;						
		}else{
			...
			foreach($_unreadNotices as $notice):				
				if(strpos($notice->getData('description'),'So colissimo Libert')):
