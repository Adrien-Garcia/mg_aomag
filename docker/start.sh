...
localDbBackupFilename=`ls -tr ${dockerComposeFilePath}/db/${DB_BACKUP_PATH_PATTERN} | tail -1`
...
then
    ...
    dbBackupFilename=`ssh ${DB_BACKUP_SERVER} "ls -tr /opt/backup/mysql/mag5/${DB_BACKUP_PATH_PATTERN} | tail -1"`

    if [ $dbBackupFilename ]
    then
        # Download the last backup file on remote server
        echo "Download database backup file ${dbBackupFilename} from ${DB_BACKUP_SERVER}  ..."
        scp ${DB_BACKUP_SERVER}:$dbBackupFilename ${dockerComposeFilePath}/db/
    else
